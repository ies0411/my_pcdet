...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            if not db_info_path.exists():
                ...
                sampler_cfg.NUM_POINT_FEATURES = sampler_cfg.BACKUP_DB_INFO[
                    "NUM_POINT_FEATURES"
                ]
        ...
        if self.img_aug_type == "nuscenes":
            self.gt_database_data_key_img = (
                self.load_db_to_shared_memory_img() if self.use_shared_memory else None
            )
    ...
    def copy_paste_to_image_kitti(
        self, data_dict, crop_feat, gt_number, point_idxes=None
    ):
        ...
        points_2d, depth_2d = data_dict["calib"].lidar_to_img(
            data_dict["points"][:, :3]
        )
        ...
        for _order in paste_order:
            ...
            if "cover" in kitti_img_aug_use_type:
                ...
                depth_mask[_box2d[1] : _box2d[3], _box2d[0] : _box2d[2], 0] = (
                    corners_lidar[_order, :, 0].min()
                )
                depth_mask[_box2d[1] : _box2d[3], _box2d[0] : _box2d[2], 1] = (
                    corners_lidar[_order, :, 0].max()
                )
        ...
        new_mask = paste_mask[points_2d[:, 1], points_2d[:, 0]] == (
            point_idxes + gt_number
        )
        if False:  # self.keep_raw:
            ...
            raw_mask = (
                raw_fg[points_2d[:, 1], points_2d[:, 0]]
                | raw_bg[points_2d[:, 1], points_2d[:, 0]]
            )
    ...
    def copy_paste_to_image_nuscenes(
        self, data_dict, crop_feat, gt_number, point_idxes=None
    ):
        ...
        for _order in paste_order:
            ...
            h = _box2d[3] - _box2d[1]
            w = _box2d[2] - _box2d[0]
            image[idx][_box2d[1] : _box2d[3], _box2d[0] : _box2d[2]] = (
                self.img_aug_mixup * crop_feat[_order][:h, :w]
                + (1 - self.img_aug_mixup)
                * image[idx][_box2d[1] : _box2d[3], _box2d[0] : _box2d[2]]
            )
        # recover crop resize
        crop_imgs = []
    ...
    def collect_image_crops_kitti(
        ...
        if True:  # self.point_refine:
            ...
            box3d_rect = box_utils.corners_rect_to_camera(box3d_coord_rect).reshape(
                1, -1
            )
            box3d_lidar = box_utils.boxes3d_kitti_camera_to_lidar(
                box3d_rect, data_dict["calib"]
            )
        ...
        raw_box = np.concatenate(
            [raw_center - new_shape / 2, raw_center + new_shape / 2]
        ).astype(np.int)
        ...
        if (raw_box[2] - raw_box[0]) != new_shape[0] or (
            raw_box[3] - raw_box[1]
        ) != new_shape[1]:
    ...
    def sample_gt_boxes_2d_kitti(self, data_dict, sampled_boxes, valid_mask):
        ...
        boxes3d_camera = box_utils.boxes3d_lidar_to_kitti_camera(
            sampled_boxes, data_dict["calib"]
        )
    ...
    def sample_gt_boxes_2d_nuscenes(
        self, data_dict, sampled_boxes, valid_mask, sampled_dict
    ):
        sampled_boxes2d = torch.Tensor(
            np.stack([x["box2d_camera"] for x in sampled_dict], axis=0).astype(
                np.float32
            )
        )
        existed_boxes2d = torch.Tensor(data_dict["gt_boxes2d"])
        ...
        for k in range(6):
            ...
            iou2d1 = (
                box_utils.pairwise_iou(view_sampled_boxes2d, view_existed_boxes2d)
                .cpu()
                .numpy()
            )
            iou2d2 = (
                box_utils.pairwise_iou(view_sampled_boxes2d, view_sampled_boxes2d)
                .cpu()
                .numpy()
            )
            iou2d2[
                range(view_sampled_boxes2d.shape[0]),
                range(view_sampled_boxes2d.shape[0]),
            ] = 0
            iou2d1 = iou2d1 if iou2d1.shape[1] > 0 else iou2d2

            view_mask = view_mask.cpu().numpy().astype(bool)
            ret_valid_mask[view_mask] = (
                (iou2d1.max(axis=1) < self.img_aug_iou_thresh)
                & (iou2d2.max(axis=1) < self.img_aug_iou_thresh)
    ...
    def sample_gt_boxes_2d(
        self, data_dict, sampled_boxes, valid_mask, sampled_dict=None
    ):
        mv_height = None
        ...
        if self.img_aug_type == "kitti":
            ...
            sampled_boxes2d, mv_height, ret_valid_mask = (
                self.sample_gt_boxes_2d_nuscenes(
                    data_dict, sampled_boxes, valid_mask, sampled_dict
                )
            )
        else:
    ...
    def initilize_image_aug_dict(self, data_dict, gt_boxes_mask):
        ...
        if self.img_aug_type is None:
            ...
            gt_crops2d = [
                data_dict["images"][_x[1] : _x[3], _x[0] : _x[2]] for _x in gt_boxes2d
            ]
            ...
            gt_boxes2d = data_dict["gt_boxes2d"][gt_boxes_mask].astype(np.int)
            gt_crops2d = [
                data_dict["ori_imgs"][_x[-1]][_x[1] : _x[3], _x[0] : _x[2]]
                for _x in gt_boxes2d
            ]
    ...
    def collect_image_crops(
        ...
        if self.img_aug_type == "kitti":
            ...
            if self.use_shared_memory:
                ...
                img_crop2d = deepcopy(
                    gt_database_data_img[img_start_offset:img_end_offset]
                )[:, :3].reshape(img_shape)
            else:
    ...
    def copy_paste_to_image(self, img_aug_gt_dict, data_dict, points):
        if self.img_aug_type == "kitti":
            ...
            data_dict["gt_boxes2d"] = np.concatenate(
                [
                    img_aug_gt_dict["gt_boxes2d"],
                    np.array(img_aug_gt_dict["crop_boxes2d"]),
                ],
                axis=0,
            )
            data_dict = self.copy_paste_to_image_kitti(
                data_dict,
                img_aug_gt_dict["gt_crops2d"],
                img_aug_gt_dict["gt_number"],
                point_idxes,
            )
            ...
            data_dict = self.copy_paste_to_image_nuscenes(
                data_dict, img_aug_gt_dict["gt_crops2d"], img_aug_gt_dict["gt_number"]
            )
        else:
            raise NotImplementedError
        return data_dict
    ...
    def add_sampled_boxes_to_scene(
        ...
        if self.use_shared_memory:
            ...
            if self.img_aug_type == "nuscenes":
                gt_database_data_img = SharedArray.attach(
                    f"shm://{self.gt_database_data_key_img}"
                )
        ...
        if (
            ...
            if self.sampler_cfg.get("FILTER_OBJ_POINTS_BY_TIMESTAMP", False):
                min_time = min(
                    self.sampler_cfg.TIME_RANGE[0], self.sampler_cfg.TIME_RANGE[1]
                )
                max_time = max(
                    self.sampler_cfg.TIME_RANGE[0], self.sampler_cfg.TIME_RANGE[1]
                )
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group["sample_num"]) > 0:
                ...
                existed_boxes = np.concatenate(
                    (existed_boxes, valid_sampled_boxes[:, : existed_boxes.shape[-1]]),
                    axis=0,
                )
        ...
        if total_valid_sampled_dict.__len__() > 0:
            sampled_gt_boxes2d = (
                np.concatenate(sampled_gt_boxes2d, axis=0)
                if len(sampled_gt_boxes2d) > 0
                else None
            )
            sampled_mv_height = (
                np.concatenate(sampled_mv_height, axis=0)
                if len(sampled_mv_height) > 0
                else None
            )
