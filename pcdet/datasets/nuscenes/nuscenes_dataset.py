...
#         save_path = Path('/mnt/nas3/Data/nuScenes').resolve()
...
class NuScenesDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(
        self, used_classes=None, max_sweeps=10, with_cam_gt=False, share_memory=False
    ):
        import torch
        ...
        all_db_infos = {}
        if share_memory:
            db_data_save_path_lidar = (
                self.root_path / f"nuscenes_{max_sweeps}sweeps_withvelo_lidar.npy"
            )
            lidar_offset_cnt = 0
            stacked_gt_lidar = []
        if with_cam_gt:
            img_database_save_path = (
                self.root_path / f"img_gt_database_{max_sweeps}sweeps_withvelo"
            )
            img_database_save_path.mkdir(parents=True, exist_ok=True)
            if share_memory:
                db_data_save_path_img = (
                    self.root_path / f"nuscenes_{max_sweeps}sweeps_withvelo_img.npy"
                )
                img_offset_cnt = 0
                stacked_gt_img = []
        ...
        for idx in tqdm(range(len(self.infos))):
            ...
            for i in range(gt_boxes.shape[0]):
                ...
                if (used_classes is None) or gt_names[i] in used_classes:
                    ...
                    if with_cam_gt:
                        img_db_path = str(
                            img_filepath.relative_to(self.root_path)
                        )  # gt_database/xxxxx.png
                        db_info.update(
                            {
                                "box2d_camera": gt_boxes_2d[i],
                                "img_path": img_db_path,
                                "img_shape": object_img_patches[i].shape,
                            }
                        )
                        if share_memory:
                            flatten_img_patches = object_img_patches[i].reshape(-1, 3)
                            stacked_gt_img.append(flatten_img_patches)
                            db_info["global_data_offset_img"] = [
                                img_offset_cnt,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use camera gt database or not",
    )
    parser.add_argument(
        "--share_memory",
        action="store_true",
