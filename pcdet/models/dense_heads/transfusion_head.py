...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        if self.model_cfg.LOSS_CONFIG.get("LOSS_IOU_REG", False):
            ...
            self.loss_iou_reg_weight = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS[
                "iou_reg_weight"
            ]
    ...
    def predict(self, inputs):
        ...
        if self.query_local:
            ...
            top_proposals_key_x = (
                top_proposals_x[:, :, None, None]
                + self.query_r_coor_x[None, None, :, :]
            )
            top_proposals_key_y = (
                top_proposals_y[:, :, None, None]
                + self.query_r_coor_y[None, None, :, :]
            )
            top_proposals_key_x = torch.clamp(
                top_proposals_key_x, min=0, max=x_grid - 1
            )
            top_proposals_key_y = torch.clamp(
                top_proposals_key_y, min=0, max=y_grid - 1
            )
            # bs, num_proposals, key_num
            top_proposals_key_index = top_proposals_key_x.view(
                batch_size, top_proposals_key_x.shape[1], -1
            ) * x_grid + top_proposals_key_y.view(
                batch_size, top_proposals_key_y.shape[1], -1
            )
            ...
            query_feat = query_feat_T.reshape(batch_size, num_proposals, -1).permute(
                0, 2, 1
            )
        else:
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        bbox_targets = torch.zeros(
            [num_proposals, self.code_size], device=center.device
        )
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, **kwargs):
        ...
        if "iou" in pred_dicts.keys() or self.model_cfg.LOSS_CONFIG.get(
            "LOSS_IOU_REG", False
        ):
            bbox_targets_for_iou = bbox_targets.permute(0, 2, 1)
            batch_box_targets_for_iou = self.decode_bbox_from_pred(
                bbox_targets_for_iou[:, 6:8, :].clone(),
                bbox_targets_for_iou[:, 3:6, :].clone(),
                bbox_targets_for_iou[:, 0:2, :].clone(),
                bbox_targets_for_iou[:, 2:3, :].clone(),
            )  # (B, Proposal, 7)
            ...
            batch_box_preds = self.decode_bbox_from_pred(
                rot.clone(), dim.clone(), center.clone(), height.clone()
            )  # (B, Proposal, 7)
            ...
            if "iou" in pred_dicts.keys():
                ...
                batch_box_targets_for_iou = (
                    batch_box_targets_for_iou.detach()
                )  # (B, Proposal, 7)
            ...
            if self.model_cfg.LOSS_CONFIG.get("LOSS_IOU_REG", False):
                iou_reg_loss = loss_utils.calculate_iou_reg_loss_transfusionhead(
                    ...
                    gt_boxes=batch_box_targets_for_iou,
                    weights=bbox_weights[:, :, :7],
                    num_pos=num_pos,
                )
                loss_all += iou_reg_loss * self.loss_iou_reg_weight
                loss_dict["loss_iou_reg"] = (
                    iou_reg_loss.item() * self.loss_iou_reg_weight
                )
    ...
    def decode_bbox_from_pred(self, rot, dim, center, height, head_index=None):
        ...
        if head_index is not None:
            center[:, 0, :] = (
                center[:, 0, :]
                * self.feature_map_stride[head_index]
                * self.voxel_size[0]
                + self.point_cloud_range[0]
            )
            center[:, 1, :] = (
    ...
    def get_bboxes(self, preds_dicts):
        ...
        for i in range(batch_size):
            ...
            # IOU refine
            if (
                self.model_cfg.POST_PROCESSING.get("USE_IOU_TO_RECTIFY_SCORE", False)
                and batch_iou is not None
            ):
                ...
                scores = torch.pow(scores, 1 - IOU_RECTIFIER[labels]) * torch.pow(
                    pred_iou, IOU_RECTIFIER[labels]
                )
